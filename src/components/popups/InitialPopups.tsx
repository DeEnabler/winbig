...
export function InitialPopups() {
  ...
  useEffect(() => {
    ...
    // Show affiliate popup first if not seen
    if (seenAffiliatePopup !== "true") {
      setShowAffiliatePopup(true);
    } else if (agreedToWarning !== "true") {
      // Show warning popup if affiliate was seen but warning not agreed to
  ...
  const handleAffiliateClose = () => {
    ...
    // After closing affiliate popup, check if warning should be shown
    const agreedToWarning = localStorage.getItem("agreedToWarning");
