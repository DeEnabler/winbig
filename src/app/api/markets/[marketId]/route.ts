...
export async function GET(
  ...
  try {
    ...
    if (!metaData || Object.keys(metaData).length === 0) {
      console.warn(`[API /markets/${marketId}] No metadata found in Redis for key ${metaKey}.`);
    } else {
    ...
    const overallEndTimeOnErrorCatch = Date.now();
    console.log(`[API /markets/${marketId}] Total execution time on error: ${overallEndTimeOnErrorCatch - overallStartTime}ms`);
